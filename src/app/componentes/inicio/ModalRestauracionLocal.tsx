...
function LugarCard({ lugar, onToggleGuardado, guardado }: { lugar: LugarLocal; onToggleGuardado: (lugar: LugarLocal) => void; guardado: boolean }) {
  ...
  return (
    <article className="overflow-hidden rounded-[28px] border border-[#eef2f7] bg-white shadow-[0_12px_30px_rgba(15,23,42,0.06)]">
      <div className="relative min-h-[128px] bg-gradient-to-br from-[#fff4ef] via-[#f8fafc] to-[#eef2ff]">
        {lugar.imagen ? (
          ...
          <div className="flex h-36 flex-col items-center justify-center px-5 text-center">
            ...
            <p className="mt-3 text-[11px] font-black uppercase tracking-[0.18em] text-[#ff5a36]">{lugar.categoria || "Restauración"}</p>
          </div>
        )}
      ...
      <div className="p-4">
        ...
        <p className="mt-1 text-xs font-bold text-[#64748b]">{lugar.categoria || "Restauración"}</p>

        {rating && <p className="mt-2 text-xs font-black text-[#ff5a36]">{rating}</p>}
        {lugar.direccion && <p className="mt-2 text-sm font-semibold leading-5 text-[#475569]">{lugar.direccion}</p>}
        {lugar.descripcion && <p className="mt-3 line-clamp-3 text-sm leading-6 text-[#667085]">{lugar.descripcion}</p>}
...
export default function ModalRestauracionLocal({ abierto, onClose, ciudadInicial = "" }: Props) {
  ...
  async function buscar(event?: FormEvent<HTMLFormElement>) {
    ...
    if (!ciudadFinal) {
      setError("Escribe una ciudad para buscar restaurantes.");
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/45 px-4 py-6 backdrop-blur-sm">
      <div className="flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-[34px] bg-white shadow-2xl">
        <div className="sticky top-0 z-10 mb-0 flex items-start justify-between gap-4 border-b border-[#eef2f7] bg-white/95 p-5 backdrop-blur sm:p-6">
          <div>
        ...
        {!buscando && !error && resultados.length === 0 && (
          <div className="mt-5 rounded-[28px] border border-dashed border-[#d9dee8] bg-[#fcfcfd] p-5 text-center">
            ...
            <p className="mt-2 text-sm font-semibold leading-6 text-[#667085]">La búsqueda se hace bajo demanda para no saturar el inicio y para mantener el recomendador turístico separado de la restauración práctica.</p>
          </div>
        )}
        </div>
      </div>
