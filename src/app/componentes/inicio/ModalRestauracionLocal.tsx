...
const ciudades = [
  "Madrid", "Barcelona", "Valencia", "Sevilla", "Málaga", "Granada", "Córdoba", "Bilbao", "San Sebastián", "Santander", "Oviedo", "Gijón", "Zaragoza", "Toledo", "Alicante", "Murcia", "Palma de Mallorca", "Ibiza", "Tenerife", "Las Palmas de Gran Canaria", "Santiago de Compostela", "A Coruña", "Vigo", "Cádiz", "Huelva", "Almería", "Logroño", "Pamplona", "Cáceres", "Mérida", "Valladolid", "Salamanca", "Segovia", "Cuenca", "Girona", "Tarragona", "Lleida"
];
...
const fuentes: Array<{ value: FuenteLugarLocal; label: string }> = [
  ...
  { value: "cache", label: "Guardados SpainWay" },
];

type Props = { abierto: boolean; onClose: () => void; ciudadInicial?: string };
...
function textoSeguro(value?: string | null): string | null {
...
function textoRating(lugar: LugarLocal): string | null {
  ...
  const rating = lugar.rating ? `${Number(lugar.rating).toFixed(1)}★` : null;
  const reviews = lugar.reviews ? `${lugar.reviews} reseñas` : null;
  return [rating, reviews].filter(Boolean).join(" · ");
}

function RestauranteCard({ lugar, guardado, onToggleGuardado }: { lugar: LugarLocal; guardado: boolean; onToggleGuardado: (lugar: LugarLocal) => void }) {
  const rating = textoRating(lugar);
  const descripcion = textoSeguro(lugar.descripcion);
  const direccion = textoSeguro(lugar.direccion);
  const categoria = textoSeguro(lugar.categoria) ?? "Restauración";

  return (
    <article className="overflow-hidden rounded-[28px] border border-[#eef2f7] bg-white shadow-[0_12px_30px_rgba(15,23,42,0.06)]">
      <div className="relative min-h-[132px] bg-gradient-to-br from-[#fff4ef] via-[#f8fafc] to-[#eef2ff]">
        {lugar.imagen ? (
          ...
          <div className="flex h-36 flex-col items-center justify-center px-5 text-center">
            ...
            <p className="mt-3 text-[11px] font-black uppercase tracking-[0.18em] text-[#ff5a36]">{categoria}</p>
          </div>
        )}
      ...
      <div className="p-4">
        ...
        <p className="mt-1 text-xs font-bold text-[#64748b]">{categoria}</p>
        ...
        <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2">
          <button type="button" onClick={() => onToggleGuardado(lugar)} className={`rounded-2xl px-4 py-3 text-center text-xs font-black ${guardado ? "bg-[#ecfdf3] text-[#027a48]" : "bg-[#f8fafc] text-[#0f172a]"}`}>
            {guardado ? "Guardado" : "Guardar"}
          </button>
          <a href={lugar.directionsUrl} target="_blank" rel="noreferrer" className="rounded-2xl bg-[#fff4ef] px-4 py-3 text-center text-xs font-black text-[#ff5a36]">Cómo llegar</a>
          <a href={lugar.googleUrl} target="_blank" rel="noreferrer" className="rounded-2xl bg-[#111827] px-4 py-3 text-center text-xs font-black text-white sm:col-span-2">Ver en Google</a>
          {lugar.url && <a href={lugar.url} target="_blank" rel="noreferrer" className="rounded-2xl bg-[#f8fafc] px-4 py-3 text-center text-xs font-black text-[#475569] sm:col-span-2">Ver fuente externa</a>}
        </div>
      </div>
    </article>
  );
...
export default function ModalRestauracionLocal({ abierto, onClose, ciudadInicial = "Valencia" }: Props) {
  const [ciudad, setCiudad] = useState(ciudadInicial || "Valencia");
  ...
  async function buscar(event?: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(null);
      setWarnings([]);
      const response = await buscarLugaresLocales({ ciudad, fecha, tipo, fuente, radiusKm: 12, limit: 18 });
      setResultados(Array.isArray(response.items) ? response.items : []);
      setWarnings(Array.isArray(response.warnings) ? response.warnings : []);
    } catch (err) {
      console.error(err);
      setError("No se pudieron buscar sitios para comer. Revisa el backend y la ruta /api/lugares-locales.");
    } finally {
      setBuscando(false);
    }
  }
  ...
  function toggleGuardado(lugar: LugarLocal) {
    const next = isRestauranteGuardado(lugar.id) ? quitarRestauranteLocal(lugar.id) : guardarRestauranteLocal(lugar);
    setGuardados(next);
  }
      ...
      <div className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-[34px] bg-white p-5 shadow-2xl sm:p-6">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm font-semibold leading-6 text-[#64748b]">Combina Google Local, Google Maps y Tripadvisor para encontrar restaurantes reales por ciudad, fecha y tipo de plan.</p>
          </div>
        ...
        <form onSubmit={buscar} className="rounded-[30px] border border-[#eef2f7] bg-[#f8fafc] p-4">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            ...
            <label className="block">
              ...
              <input type="date" value={fecha} onChange={(event) => setFecha(event.target.value)} className="mt-2 h-13 w-full rounded-2xl border border-[#d9dee8] bg-white px-4 text-sm font-bold text-[#0f172a] outline-none focus:border-[#ff5a36]" />
            </label>
            <label className="block">
              <span className="text-xs font-black uppercase tracking-[0.16em] text-[#94a3b8]">Tipo de plan</span>
              <select value={tipo} onChange={(event) => setTipo(event.target.value as TipoLugarLocal)} className="mt-2 h-13 w-full rounded-2xl border border-[#d9dee8] bg-white px-4 text-sm font-bold text-[#0f172a] outline-none focus:border-[#ff5a36]">
            ...
            <label className="block">
              ...
              <select value={fuente} onChange={(event) => setFuente(event.target.value as FuenteLugarLocal)} className="mt-2 h-13 w-full rounded-2xl border border-[#d9dee8] bg-white px-4 text-sm font-bold text-[#0f172a] outline-none focus:border-[#ff5a36]">
                {fuentes.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}
              </select>
            </label>
          </div>
          ...
          <div className="mt-4 rounded-2xl bg-white p-4">
            ...
            <p className="mt-1 text-sm font-semibold leading-6 text-[#667085]">{tipoSeleccionado?.hint}. Si una fuente falla, SpainWay sigue con el resto y no rompe la búsqueda.</p>
          </div>
        ...
        {warnings.length > 0 && <div className="mt-4 rounded-3xl bg-[#fff7ed] p-4 text-sm font-semibold leading-6 text-[#9a3412]">{warnings.slice(0, 3).map((item) => <p key={item}>• {item}</p>)}</div>}
        ...
        {resultados.length > 0 && (
          <section className="mt-5">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {resultados.map((lugar) => <RestauranteCard key={lugar.id} lugar={lugar} guardado={guardadosIds.has(lugar.id)} onToggleGuardado={toggleGuardado} />)}
            </div>
          </section>
        )}
      </div>
