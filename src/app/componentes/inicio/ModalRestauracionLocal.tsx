...
const tipos: Array<{ value: TipoLugarLocal; label: string; hint: string }> = [
  { value: "comer_bien", label: "Comer bien", hint: "Restaurantes recomendados y con buena valoración" },
  { value: "cenar", label: "Cenar", hint: "Opciones para tarde/noche" },
  { value: "tapas", label: "Tapas", hint: "Bares, tabernas y cocina informal" },
  { value: "cafe_brunch", label: "Café / brunch", hint: "Cafeterías, desayunos y brunch" },
  { value: "tipico_local", label: "Típico local", hint: "Comida propia de la zona" },
  ...
  return (
    <article className="overflow-hidden rounded-[28px] border border-[#eef2f7] bg-white shadow-[0_12px_30px_rgba(15,23,42,0.06)]">
      ...
      <div className="p-4">
        ...
        <p className="mt-1 text-xs font-bold text-[#64748b]">{limpiarTexto(lugar.categoria) || "Restauración"}</p>
        ...
        <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2">
          ...
          <a href={lugar.directionsUrl} target="_blank" rel="noreferrer" className="rounded-2xl bg-[#fff4ef] px-4 py-3 text-center text-xs font-black text-[#ff5a36]">
            Cómo llegar
          </a>

          <a href={lugar.googleUrl} target="_blank" rel="noreferrer" className="rounded-2xl bg-[#111827] px-4 py-3 text-center text-xs font-black text-white sm:col-span-2">
            Ver en Google
          </a>

          {lugar.url && (
            <a href={lugar.url} target="_blank" rel="noreferrer" className="rounded-2xl bg-[#f8fafc] px-4 py-3 text-center text-xs font-black text-[#475569] sm:col-span-2">
              Ver fuente externa
            </a>
          )}
        </div>
...
const ciudadPorDefecto = (CIUDADES_SPAINWAY as readonly string[]).includes(ciudadInicial)
  ...
  async function buscar(event?: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setWarnings((response.items?.length ?? 0) > 0 ? avisos.filter((item) => !/no respond/i.test(item)).slice(0, 2) : avisos);
    } catch (err) {
      console.error(err);
        ...
        <div className="shrink-0 border-b border-[#eef2f7] bg-white p-5 sm:p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <button
                type="button"
                onClick={() => setMostrarGuardados((value) => !value)}
                className="mt-4 rounded-2xl bg-[#111827] px-4 py-3 text-xs font-black text-white"
              >
                {mostrarGuardados ? "Volver a la búsqueda" : `Ver restaurantes guardados (${guardados.length})`}
              </button>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6">
              ...
              <div className="mb-3 flex items-end justify-between gap-3">
                <div>
                  ...
                  <h3 className="mt-1 text-xl font-black text-[#0f172a]">
                    {mostrarGuardados ? `${guardados.length} restaurantes guardados` : `${resultados.length} sitios encontrados`}
                  </h3>
                </div>
              </div>
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                {listaVisible.map((lugar) => <LugarCard key={lugar.id} lugar={lugar} guardado={guardadosIds.has(lugar.id)} onToggleGuardado={toggleGuardado} />)}
              </div>
            </section>
          )}
          ...
          {!buscando && !error && listaVisible.length === 0 && (
            <div className="mt-5 rounded-[28px] border border-dashed border-[#d9dee8] bg-[#fcfcfd] p-5 text-center">
              ...
              <p className="mt-3 text-base font-black text-[#0f172a]">{mostrarGuardados ? "Aún no tienes restaurantes guardados" : "Busca restaurantes reales cuando los necesites"}</p>
              <p className="mt-2 text-sm font-semibold leading-6 text-[#667085]">
                {mostrarGuardados ? "Guarda un restaurante desde los resultados y aparecerá aquí para consultarlo más tarde." : "La búsqueda se hace bajo demanda para mantener el inicio limpio y útil."}
              </p>
            </div>
          )}
        </div>
